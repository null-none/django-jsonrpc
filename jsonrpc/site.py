    ...
    if type(D["params"]) == Object:
        ...
        if len(keys) != len(D["params"]):
            raise InvalidParamsError(
                "Not enough params provided for %s" % method.json_sig
            )
        for k in keys:
            if not k in D["params"]:
                print("\n\n\n\nSHITTER SHITTER", k, D, "\n\n\n\n")
                raise InvalidParamsError(
                    "%s is not a valid parameter for %s" % (k, method.json_sig)
                )
        ...
        try:
            ...
            raise InvalidParamsError(
                "Too many params provided for %s" % method.json_sig
            )
        else:
...
class JSONRPCSite(object):
    ...
    def validate_get(self, request, method):
        encode_get_params = lambda r: dict(
            [(k, v[0] if len(v) == 1 else v) for k, v in r]
        )
    ...
    def response_dict(
        self, request, D, is_batch=False, version_hint="1.0", json_encoder=None
    ):
        json_encoder = json_encoder or self.json_encoder
        version = version_hint
        response = self.empty_response(version=version)
        apply_version = {
            "2.0": lambda f, r, p: f(r, **encode_kw(p))
            if type(p) is dict
            else f(r, *p),
        ...
        try:
            ...
            if "id" in D and D["id"] is not None:  # regular request
                ...
                is_batch
            ):  # notification, not ok in a batch format, but happened anyway
                raise InvalidRequestError
            ...
            builtin_types = (
                (dict, list, set, NoneType, bool, six.text_type)
                + six.integer_types
                + six.string_types
            )
    ...
    def dispatch(self, request, method="", json_encoder=None):
        ...
        try:
            ...
            if type(D) is list:
                response = [
                    self.response_dict(
                        request, d, is_batch=True, json_encoder=json_encoder
                    )[0]
                    for d in D
                ]
                status = 200
                ...
                response, status = self.response_dict(
                    request, D, json_encoder=json_encoder
                )
                if response is None and (
                    not "id" in D or D["id"] is None
                ):  # a notification
        ...
        return HttpResponse(
            json_rpc, status=status, content_type="application/json-rpc"
        )
