...
def _call(host, req):
    return loads(
        urllib_request.urlopen(host, dumps(req).encode("utf-8")).read().decode("utf-8")
    )
            ...
            try:
                ...
                try:
                    User.objects.create_user(
                        username="sammeh", email="sam@rf.com", password="password"
                    ).save()
                except:
                    pass
...
@jsonrpc_method(
    "jsonrpc.checkedEcho(string=str, string2=str) -> str", safe=True, validate=True
)
...
class JSONRPCFunctionalTests(unittest.TestCase):
    def test_method_parser(self):
        working_sigs = [
            ...
            (
                "jsonrpc.methodName(str, str, str ) ",
                "jsonrpc.methodName",
                OrderedDict([("a", str), ("b", str), ("c", str)]),
                Any,
            ),
            (
                "jsonrpc.methodName(str, b=str, c=str)",
                "jsonrpc.methodName",
                OrderedDict([("a", str), ("b", str), ("c", str)]),
                Any,
            ),
            (
                "jsonrpc.methodName(str, b=str) -> dict",
                "jsonrpc.methodName",
                OrderedDict([("a", str), ("b", str)]),
                dict,
            ),
            (
                "jsonrpc.methodName(str, str, c=Any) -> Any",
                "jsonrpc.methodName",
                OrderedDict([("a", str), ("b", str), ("c", Any)]),
                Any,
            ),
    ...
    def test_validate_args_any(self):
        ...
        self.assertTrue(validate_params(M, {"params": [["omg"], ["wtf"]]}) is None)
        self.assertTrue(
            validate_params(M, {"params": {"s1": "omg", "s2": "wtf"}}) is None
        )
...
class ServiceProxyTest(JSONServerTestCase):
    def test_positional_args(self):
        ...
        try:
            ...
            self.assertTrue(
                e.args[0] == "Unsupported arg type for JSON-RPC 1.0 "
